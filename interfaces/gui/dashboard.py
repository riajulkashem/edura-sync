...
from interfaces.gui.ui_utils import (
    create_window, setup_styles, create_notebook,
    create_tab, load_icon, create_button
)
...
class DashboardGUI:
    ...
    def __init__(
            self, root, app, device_repo, user_repo, notification_service,
            settings_repo=None, api_client=None, security=None
    ):
        ...
        self.in_time_process = None
        self.out_time_process = None
    ...
    def _create_settings_tab(self, frame):
        ...
        try:
            ...
            if not self.settings_repo:
                ttk.Label(frame, text="Error: Settings repository not available", foreground="red").grid(
            ...
            try:
                ...
                settings = None
                ttk.Label(frame, text=f"Error loading settings: {str(e)}", foreground="red").grid(
                    row=0, column=0, columnspan=2, pady=20
                )
            ...
            if self.security:
                ...
                ttk.Label(frame, text="Save functionality not available - missing security component",
                          foreground="red").grid(row=10, column=0, columnspan=2, pady=20)
            ...
            ttk.Label(frame, text=f"Error creating settings interface: {str(e)}", foreground="red").grid(
                row=0, column=0, columnspan=2, pady=20
            )
    ...
    def _create_api_settings_section(self, parent, settings, row_start):
        ...
        self.cloud_api_url.grid(row=row_start + 1, column=1, sticky="ew", padx=10, pady=5)
        ...
        self.institute_id.grid(row=row_start + 4, column=1, sticky="ew", padx=10, pady=5)
    ...
    def _create_time_settings_section(self, parent, settings, row_start):
        """Create time settings form fields section."""
        # Section header
        ttk.Label(parent, text="Time Settings", font=("TkDefaultFont", 12, "bold")).grid(
            row=row_start, column=0, columnspan=2, sticky="w", padx=10, pady=(20, 5)
        )

        # In Time
        ttk.Label(parent, text="In Time Process:").grid(
            row=row_start + 1, column=0, sticky="w", padx=10, pady=5
        )
        self.in_time_process = ttk.Entry(parent, width=10)
        self.in_time_process.grid(row=row_start + 1, column=1, sticky="w", padx=10, pady=5)
        ttk.Label(parent, text="Format: HH:MM (24-hour)").grid(
            row=row_start + 1, column=1, sticky="e", padx=10, pady=5
        )

        default_in_time = "09:00"
        if settings and hasattr(settings, 'in_time_process') and settings.in_time_process:
            self.in_time_process.insert(0, settings.in_time_process.strftime("%H:%M"))
        else:
            self.in_time_process.insert(0, default_in_time)

        # Out Time
        ttk.Label(parent, text="Out Time Process:").grid(
            row=row_start + 2, column=0, sticky="w", padx=10, pady=5
        )
        self.out_time_process = ttk.Entry(parent, width=10)
        self.out_time_process.grid(row=row_start + 2, column=1, sticky="w", padx=10, pady=5)
        ttk.Label(parent, text="Format: HH:MM (24-hour)").grid(
            row=row_start + 2, column=1, sticky="e", padx=10, pady=5
        )

        default_out_time = "17:00"
        if settings and hasattr(settings, 'out_time_process') and settings.out_time_process:
            self.out_time_process.insert(0, settings.out_time_process.strftime("%H:%M"))
        else:
            self.out_time_process.insert(0, default_out_time)
    ...
    def _create_credits_tab(self, frame):
        ...
        try:
            ...
            # Developer info
            ttk.Label(
                frame, text="Developed by Riajul Kashem", font=("TkDefaultFont", 10, "bold")
            ).pack(pady=(20, 0))
    ...
    def _update_dashboard_content(self):
        ...
        try:
            ...
            if not self.device_repo:
                ttk.Label(self.dashboard_content, text="Error: Device repository not available",
    ...
    def _build_status_section(self, devices):
        ...
        # Section header
        ttk.Label(self.dashboard_content, text="Status Overview", font=("TkDefaultFont", 12, "bold")).pack(
        ...
        if self.settings_repo:
            try:
                ...
                # Last Sync Time
                ttk.Label(status_frame, text="Last Synced:").grid(row=0, column=0, sticky="w", padx=5, pady=2)
                ...
                ttk.Label(status_frame, text=last_sync_text).grid(row=0, column=1, sticky="w", padx=5, pady=2)

                # Last Post Time
                ttk.Label(status_frame, text="Last Post Cloud:").grid(row=1, column=0, sticky="w", padx=5, pady=2)
                ...
                ttk.Label(status_frame, text=last_post_text).grid(row=1, column=1, sticky="w", padx=5, pady=2)

                # Pending Attendance
                ttk.Label(status_frame, text="Pending Attendance For Post:").grid(
                    row=2, column=0, sticky="w", padx=5, pady=2
                )
                pending_count = settings.attendance_pending if hasattr(settings, 'attendance_pending') else 0
                ttk.Label(status_frame, text=str(pending_count)).grid(row=2, column=1, sticky="w", padx=5, pady=2)

                # User count
                ttk.Label(status_frame, text="Total Users:").grid(row=3, column=0, sticky="w", padx=5, pady=2)
                total_users = self.user_repo.count() if self.user_repo else 0
                ttk.Label(status_frame, text=str(total_users)).grid(row=3, column=1, sticky="w", padx=5, pady=2)

                # Device count
                ttk.Label(status_frame, text="Total Devices:").grid(row=4, column=0, sticky="w", padx=5, pady=2)
                total_devices = len(devices) if devices else 0
                online_devices = sum(1 for device in devices if device.status == "Online") if devices else 0
                ttk.Label(status_frame, text=f"{online_devices} online / {total_devices} total").grid(
                    row=4, column=1, sticky="w", padx=5, pady=2
                )

            except Exception as e:
                ttk.Label(status_frame, text=f"Error loading settings: {str(e)}", foreground="red").grid(
                    row=0, column=0, columnspan=2, padx=5, pady=2
                )
                self.logger.error(f"Error loading settings for dashboard: {e}")
        else:
            ttk.Label(status_frame, text="Settings information not available", foreground="red").grid(
                row=0, column=0, columnspan=2, padx=5, pady=2
            )
    ...
    def _build_devices_section(self, devices):
        ...
        # Section header
        ttk.Label(self.dashboard_content, text="Device Status", font=("TkDefaultFont", 12, "bold")).pack(
        ...
        for device in devices:
            ...
            ttk.Label(device_frame, text="●", foreground=status_color).grid(row=0, column=0, padx=5)

            # Device info
            ttk.Label(device_frame, text=f"{device.device_model} ({device.ip_address})").grid(
    ...
    def _build_actions_section(self):
        ...
        # Section header
        ttk.Label(self.dashboard_content, text="Actions", font=("TkDefaultFont", 12, "bold")).pack(
            anchor="w", padx=10, pady=(15, 5)
        )
    ...
    def _perform_action(self, action_func, action_name):
        ...
        if not action_func:
            self.notification_service.notify(
                "Error", f"Cannot perform {action_name}: required component not available", "error"
            )
            return False
        ...
        try:
            ...
            if hasattr(self, "status_label") and self.status_label:
                self.status_label.config(text=f"Performing {action_name}...", foreground="blue")
                # Force UI update
            ...
            if hasattr(self, "status_label") and self.status_label:
                if result:
                    self.status_label.config(text=f"{action_name} completed successfully", foreground="green")
                else:
                    error_msg = f"{action_name} failed"
                    if self.api_client and hasattr(self.api_client, 'last_error') and self.api_client.last_error:
    ...
    def _save_settings(self):
        ...
        if not self.settings_repo:
            self.notification_service.notify(
                "Error", "Cannot save settings: Settings repository not available", "error"
            )
            return
        ...
        try:
            ...
            in_time = self.in_time_process.get()
            out_time = self.out_time_process.get()
            ...
            try:
                ...
                self.notification_service.notify(
                    "Error", "Invalid time format. Please use HH:MM (24-hour format).", "error"
                )
                return
            ...
            settings_data = {
                ...
                'in_time_process': in_time_obj,
                'out_time_process': out_time_obj,
            }
            ...
            # Notify success
            self.notification_service.notify("Settings", "Settings saved successfully", "info")
            ...
            self.notification_service.notify("Error", f"Failed to save settings: {str(e)}", "error")

    def _check_api_connection(self):
        """Check connection to the API with current form values."""
        if not self.api_client:
            self._update_status_label("Cannot check connection: API client not available", "red")
            return
        ...
        try:
            ...
            if token_response.status_code == 200:
                ...
                if token:
                    ...
                    self._update_status_label("Authenticated successfully. Checking API access...", "blue")
                    if self.dashboard_win:
                        self.dashboard_win.update()
                    ...
                    self._update_status_label("Authentication failed: No token received", "red")
            else:
                self._update_status_label(
            ...
            self._update_status_label("Connection error: Could not connect to server", "red")
